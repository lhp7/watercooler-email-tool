...
def build_msg_message(
    ...
    message.internet_code_page = 65001
    message.message_code_page = 65001
    message.subject = subject
    message.body_html_text = html_body
    message.body = ""
    message.sender_name = sender_name
    message.sender_email_address = sender_email
    message.sent_representing_name = sender_name
    message.sent_representing_email_address = sender_email
    message.sent_representing_address_type = "SMTP"
    message.sender_address_type = "SMTP"

    recipients = []

    to_recipient = Recipient()
    to_recipient.display_name = recipient_email
    to_recipient.email_address = recipient_email
    to_recipient.smtp_address = recipient_email
    to_recipient.transmitable_display_name = recipient_email
    to_recipient.address_type = "SMTP"
    to_recipient.search_key = f"SMTP:{recipient_email.upper()}\x00".encode("ascii")
    ...
    for addr in _split_addresses(cc):
        ...
        r.smtp_address = addr
        r.transmitable_display_name = addr
        r.address_type = "SMTP"
        r.search_key = f"SMTP:{addr.upper()}\x00".encode("ascii")
